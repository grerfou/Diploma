...
static int glitchPercentage = 20;   // pourcentage de mots glitchés
...
void ChooseWordsToScramble(int percentage) {
    ...
    int wordsToScramble = (percentage * totalWords) / 100;
...
void UpdateLettersBug(float deltaTime) {
    timeSinceLastChange += deltaTime;
    if (timeSinceLastChange >= changeInterval) {
        timeSinceLastChange = 0.0f;
        ChooseWordsToScramble(glitchPercentage);  // On change la sélection
    }
